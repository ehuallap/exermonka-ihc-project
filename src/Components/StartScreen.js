    ...
    return (
      ...
      <div className={styles.startPage}>
        ...
        <div className={styles.test}>
          <p>Libera tu estrés esquivando los obtáculos!</p>
        </div>
        <div style={{width:"100%", height: "300px", zIndex:999}} id="character"></div>
        <div className={styles.test}>
          <p style={{marginTop: "em"}}> Si puedes verlo, puedes hacerlo!</p>
        </div>
        
        <div style={{ textAlign: "center", width: "100%", display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gridGap: 100 }}>
            <Link to="/game" className={styles.fe_pulse}> Modo clásico! </Link>
            <Link to="/calibrate" onClick={() => this.props.handlePoseNet(true)} className={styles.fe_pulse}> Modo detección! </Link>
        </div>
      </div>
		</>
    )
